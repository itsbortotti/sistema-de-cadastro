...
async function listar() {
  const rows = await prisma.hospedagem.findMany({ orderBy: { nome: 'asc' } });
  return rows.map((r) => ({ ...defaults(), ...r }));
}

async function getById(id) {
  const r = await prisma.hospedagem.findUnique({ where: { id } });
  return r ? { ...defaults(), ...r } : null;
}

async function criar(dados) {
  const data = normalizar(dados);
  const novo = await prisma.hospedagem.create({ data });
  return { ...defaults(), ...novo };
}

async function atualizar(id, dados) {
  const existente = await prisma.hospedagem.findUnique({ where: { id } });
  if (!existente) return null;
  const updateData = {};
  CAMPOS.forEach((c) => {
    if (dados[c] !== undefined) updateData[c] = String(dados[c]).trim();
  });
  const atualizado = await prisma.hospedagem.update({ where: { id }, data: updateData });
